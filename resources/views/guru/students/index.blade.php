...
<style>
    ...
    /* Class badge */
    .class-badge{display:inline-flex;align-items:center;background:rgba(103,111,157,.18);border:1px solid rgba(103,111,157,.28);color:var(--slate);font-size:.7rem;font-weight:700;letter-spacing:.04em;padding:.2rem .65rem;border-radius:100px;}
            ...
            <tbody>
                @forelse($students as $student)
                    <tr>
                        ...
                        <td>
                            @if($student->photo)
                                <img src="{{ asset('storage/' . $student->photo) }}" class="tbl-avatar" alt="{{ $student->name }}">
                            @else
                    ...
                    <tr>
                        <td colspan="5">
                            <div class="empty-state">
                                ...
                                <p class="empty-sub">Belum ada siswa yang terdaftar dalam sistem.</p>
                            </div>
                        </td>
                    </tr>
                @endforelse
    ...
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            ...
            <div class="modal-body">
                @if($student->photo)
                    <img src="{{ asset('storage/' . $student->photo) }}" class="modal-avatar-lg" alt="{{ $student->name }}">
                @else
                ...
                <div class="modal-info-row">
                    ...
                    <span class="modal-info-val accent">{{ $student->class }}</span>
                </div>
                <div class="modal-info-row">
                    <span class="modal-info-key">🔢 NO. ABSEN</span>
                    <span class="modal-info-val">{{ $student->attendance_number ?? '-' }}</span>
                </div>
            </div>
        </div>
    </div>
